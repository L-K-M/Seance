...
mode withholds read from group or others is staged owner-only. A symbolic
link, FIFO, socket or device at the target is refused rather than replaced,
since the rename would replace the node itself, not a link's target.
