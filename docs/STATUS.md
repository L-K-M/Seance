...
>>>>>>> claude/seance-server-sync-features-7g0elp-zai-search
...
<<<<<<< HEAD
  flag without a fresh `updatedAt` trips an assert rather than silently losing
  the LWW tie, and excluding on one device removes it from the other; plus
