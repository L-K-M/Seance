...
- `seance_core/test/sync_coordinator_test.dart` — domain⇄record mapping,
  ...
  device's stale copy, and excluding on one device removes it from the other.
...
- `app/seance_app/test/server_duplication_test.dart` — copy naming (numbered
  ...
  named "copy"), what a duplicate carries over (including inheriting rather
  than widening the two sync answers) and takes fresh, and the row menu's
  Duplicate entry.
- `app/seance_app/test/server_exclude_from_sync_test.dart` — the row's
  ...
  every label, so a tooltip there would be silently dropped).
