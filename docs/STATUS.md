...
- `packages/seance_core/test/sync_coordinator_test.dart` — domain⇄record mapping,
  ...
  device's stale copy (config *and* credential), a `secret:` tombstone is
  staged but never honoured against the vault (a tombstone carries no sealed
  payload, so a delete is the one thing a sync server can assert on its own,
  and a credential vault is not where to grant it that), a credential a synced
  ...
  when the copy on the sync server outranks the retraction. The same file also
  proves a server's group, colour and icon travel between devices, and that
  regrouping converges like a rename (a group is a name its members carry, not
  a record that can dangle).
