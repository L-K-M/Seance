...
| `seance_app` | Complete; `flutter analyze` clean, widget tests pass. Server list is the tab list (one terminal per server, status dot: green/grey/red + connecting spinner; resizable tiled panes, no terminal title bar in wide mode); assistant tiled sidebar when configured (⌘/Ctrl+↵ sends); inline command generator (⌘K / Ctrl+Shift+K, prefilled from the current shell line, Enter generates+inserts+closes) turns NL into a reviewed command; Settings open from a macOS menu item / ⌘, (still an in-app route); settings suggest models from the endpoint with manual fallback; failed connections show a summary + expandable connection log. Default desktop window 1800×1600. Platform folders committed. |
