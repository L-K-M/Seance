...
- `seance_protocol/test/assistant_settings_test.dart` — round-trips with keys,
  a key-less record byte-identical to one whose keys were dropped, a cleared
  field arriving cleared rather than as an empty endpoint, redaction defaulting
  on for an older writer, the singleton id being prefixed (a bare-id tombstone
  would read as a server deletion), `toString` counting keys instead of
  printing them, and a malformed key map reading as none.
...
- `seance_core/test/zai_search_test.dart` — the Z.AI MCP client against a fake
  ...
  rather than silently discarded), a paginated tool listing walked to the end,
  non-JSON stream events walked past, a non-JSON 200 failing like any other
  bad reply, and a stalled body timing out rather than hanging; plus
  `CompositeSearch` interleaving, tolerating any backend error (not just
  transport ones), raising with the original failure when all of them fail,
  and doing nothing when there are none.
...
- `seance_core/test/sync_coordinator_test.dart` — domain⇄record mapping,
  unknown-kind preservation, fail-soft apply, and tombstone dispatch; plus
  exclude-from-sync: an excluded server is retracted rather than pushed (with
  its credential), the retraction is dated at the exclusion so repeat rounds
  are no-ops, a host key is withheld only when no synced server shares the
  address, an excluded server survives both its own retraction and another
  device's stale copy (config *and* credential), a `secret:` tombstone deletes
  only a vault entry no local server still references, a credential a synced
  server shares is neither withdrawn nor frozen, the retraction is keyed
  ...
  LWW tie, and excluding on one device removes it from the other; plus
  assistant settings — the configuration and its keys reaching the other
  device, a later edit winning while idle rounds mint no new sequence numbers
  (the record is stamped from the edit, not the round), nothing published
  before the assistant is configured, and a device that has not opted in
  neither pushing nor adopting.
...
- `seance_core/test/test_connection_test.dart` — the trial connection: it
  reports *how* authentication completed, reuses the real failure summary
  verbatim, writes a pre-handshake failure (locked keyring, unreadable
  identity file) into the transcript so an expanded log doesn't stop
  mid-sentence, turns the unimplemented ssh-agent path into a sentence rather
  than "Unsupported operation:", flags a configured jump host on success *and*
  on the failure that caveat explains, and keeps a stack trace for a bug while
  leaving an expected failure readable; plus `UnpinnedHostKeyStore` reading
  existing pins through, keeping its own to the attempt, and listing a
  re-approved host once rather than twice.
- `seance_core/test/ssh_diagnostics_test.dart` — connection-log redaction (a
  keyboard-interactive answer never reaches the transcript, while the rest of
  the exchange stays legible); connection-log capture and the
...
- `app/seance_app/test/app_settings_test.dart` — also that `zaiApiKeyRef` and
  `syncAssistant` are off by default, round-trip, and read as "not configured"
  / "never published" in a settings file written before they existed.
- `app/seance_app/test/assistant_settings_sync_test.dart` — the bridge between
  `AppSettings`, the OS keystore and the record: nothing published before the
  assistant is configured, only the referenced keys published (never the sync
  token or the vault key beside them), a locked keyring costing the keys
  rather than the round, adoption writing the keys back and flagging that the
  chat provider must be rebuilt, an unknown provider name keeping the one
  configured, and a key-less record leaving the local keys alone.
- `app/seance_app/test/connection_test_report_test.dart` — the test result as
  ...
  present but folded (with a "(no log captured)" placeholder and a disabled
  Copy when there is nothing to copy).
- `app/seance_app/test/server_duplication_test.dart` — copy naming (numbered
  series, copy-of-a-copy, case- and whitespace-insensitive matching, a server
  named "copy"), what a duplicate carries over — compared as JSON against the
  source, so a field added later fails here rather than being silently reset —
  and what it takes fresh (including inheriting rather than widening the two
  sync answers); `planServerDuplication` copying the credential into a vault
  entry of its own (passphrase included), degrading a dangling reference to
  "no credential" while a locked vault fails outright; and the row menu's
  Duplicate entry, the source's identity-file grant travelling to the copy,
  and `secretStillReferenced` keeping a shared vault entry alive when one of
  its owners is deleted.
