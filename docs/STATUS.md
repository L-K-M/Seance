...
_Last updated: 2026-09-26. Séance now supports native ssh-agent
authentication and recursive saved-host ProxyJump routes. Before that,
Séance gained themes: one editable palette per
...
resize and narrow-navigation suites pass 32 tests. Coverage includes both
handles, RTL, Tab traversal and focus appearance, repeated steps before a
frame, bounds, window shrink, truthful adjustment announcements and the
...
material. `SshAgentClient` implements the bounded OpenSSH identity and signing
messages, turns each public key into an `SSHIdentity.custom` signer, requests
RSA SHA-256 signatures, and rejects malformed, oversized, trailing, or
unexpected replies. Unix connects through `$SSH_AUTH_SOCK`; Windows connects
to `\\.\pipe\openssh-ssh-agent`, with blocking named-pipe work isolated from
the app thread. New and keyless imported servers default to the agent, and
agent auth does not open the vault or consult an obsolete secret reference.

`openAuthenticatedClient` now resolves a server's full `jumpHostId` chain and
each hop's credentials before opening a socket. Missing hosts, mismatched ids,
cycles, and overlong chains therefore fail before network I/O. It opens only
the outer host directly, reaches each inner host over SSH `direct-tcpip`, and
applies that hop's authentication, TOFU decision, connection logging, timeout,
and keepalive. Closing the destination closes every forwarding channel and
parent client once. Normal sessions and Test connection share the resolver;
test-only host-key approvals remain ephemeral at every hop.
Keyboard-interactive dialogs show the trusted saved endpoint separately from
the jump host's server-supplied name, instructions, and prompts.
...
- `packages/seance_core/test/stores_probe_ssh_test.dart` — SecretVault, ConfigStore,
  ...
  engine.
- `packages/seance_core/test/ssh_agent_test.dart` and
  `ssh_agent_unix_test.dart` — OpenSSH agent framing, identities, custom
  signing, RSA SHA-256 negotiation, malformed/bounded replies, a fragmented
  real Unix-socket exchange, timeout cleanup, and actionable missing-agent
  failures.
- `packages/seance_core/test/ssh_proxy_jump_test.dart` — direct, one-hop and
  recursive routes; all route failures before network I/O; forwarding order,
  failure and constructor cleanup, and ownership of the complete
  client/channel chain.
