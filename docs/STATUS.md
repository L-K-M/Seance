...
config record, bounded at 32 KiB decoded. The binding constraint is not the
sync server's 1 MiB per record, which that clears easily, but its 8 MiB per
*request*: `SyncEngine._pushOnce` sends every dirty record in one push, so the
question is how many marked servers can go at once — around 190 all carrying a
maximum-size image, far past any real collection. Batching that push by size is
the structural fix, is a change to the sync engine rather than to this feature,
and is what a larger cap would depend on. Every import is re-encoded rather than
trusted: cropped square (the badge draws edge to edge, and letterboxing reads
as a broken image), scaled to at most 128 px, never scaled up, and stepped down
to 96 or 64 if the PNG will not fit. That is `dart:ui` throughout — the engine
already decodes every format the platform knows, so no image package was added.
A value that fails validation on read is not re-published either, so a device
never passes on a mark it could not draw as though it had accepted it.
...
620 Dart tests, 552 Flutter tests and 166 in the vendored fork pass; every
