...
- Server list: the two views are back, on the desktop rail, on tablets
  ...
  reader hears it with the header ("Connected server hidden"); a changed
  host key you declined marks the row blocked rather than failed; the
  filter's count says "↵ opens the first" again; the long-press sheet
  shows the row's second line under its name; and the filter chord no
  longer latches the field open on an empty list.
