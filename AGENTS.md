...
The macOS `MACOSX_DEPLOYMENT_TARGET` is **12.0**, not the 10.15 the Flutter
template once generated: the SDK's own CocoaPods template declares
`platform :osx, '12.0'`, so an older floor no longer builds. Anything guarded
by `#available(macOS 11.0, *)` or `#available(macOS 12.0, *)` in the runner is
therefore dead weight — the check is always true and the dead branch is still
compiled, which is how a deprecated API that was deliberately replaced gets its
warning back.
